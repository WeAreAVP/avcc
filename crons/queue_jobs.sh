...
CMD="$PHP_PATH ${BASEDIR}app/console jms-job-queue:run --env=prod > $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH"
MSG="ENV [$ENVIRONMENT]"
...
`$PHP_PATH ${BASEDIR}app/console jms-job-queue:run --env=prod >> $OUTPUT_FILE 2>&1 & echo $! > $PID_FULLPATH`
